...
@section('content')
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($service->venues->count())
                                                    @foreach ($service->venues->take(3) as $v)
                                                        <span
                                                            class="badge bg-info bg-opacity-10 text-dark border border-info mb-1"
                                                            title="Sân: {{ $v->name }}">
                                                            {{ Str::limit($v->name, 15) }}
                                                        </span> <br>
                                                    @endforeach
                                                    @if ($service->venues->count() > 3)
                                                        <small
                                                            class="text-muted fst-italic">+{{ $service->venues->count() - 3 }}
                                                            sân khác</small>
                                                    @endif
                                                @else
                                            ...
                                            <td class="fw-bold text-success">
                                                @if ($service->type == 'amenities')
                                                    <span class="badge bg-success bg-opacity-10 text-white">Miễn
                                                        phí</span>
                                                @else
                                                    @php
                                            ...
                                            <td class="text-end pe-4">
                                                {{-- Nút Chỉnh sửa: Gắn đầy đủ data-attribute --}}
                                                <button type="button" class="btn btn-sm btn-light text-primary border me-1"
                                                    title="Chỉnh sửa" data-bs-toggle="modal"
                                                    data-bs-target="#modalEditService" data-id="{{ $service->id }}"
                                                    data-name="{{ $service->name }}"
                                                    data-category-id="{{ $service->category_id }}"
                                                    data-type="{{ $service->type }}" data-unit="{{ $service->unit }}"
                                                    data-description="{{ $service->description }}" {{-- Lấy giá cao nhất làm giá tham khảo --}}
            ...
            <form action="{{ route('owner.services.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-content">
                    <div class="modal-header bg-light">
                        <h5 class="modal-title fw-bold text-primary"><i class="fas fa-plus-circle me-2"></i>Thêm Dịch vụ Mới
                        </h5>
                    ...
                    <div class="modal-body p-4">
                        <div class="row g-4">
                            ...
                            <div class="col-md-6 border-end">
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-bold">Tên dịch vụ <span
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-6 mb-3">
                                        <label class="form-label fw-bold">Loại hình <span
                                                class="text-danger">*</span></label>
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-bold">Đơn vị tính <span
                                            class="text-danger">*</span></label>
                                    <input type="text" name="unit" class="form-control" required
                                        placeholder="Vd: Chai, Lượt, Giờ...">
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-bold">Áp dụng cho sân nào? <span
                                            class="text-danger">*</span></label>
                                    <div class="card p-2 bg-light border-0" style="max-height: 150px; overflow-y: auto;">
                                        @if ($venues->count() > 0)
                                            <div class="form-check mb-1 border-bottom pb-1">
                                                ...
                                                <label class="form-check-label fw-bold" for="checkAllAdd">Chọn tất
                                                    cả</label>
                                            </div>
                                            ...
                                            @endforeach
                                        @else
                                            <span class="text-danger small">Chưa có sân nào. Vui lòng tạo sân trước.</span>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="price" id="add_price"
                                        class="form-control fw-bold text-success" required min="0" value="0">
                                    <div class="form-text small">Nếu là tiện ích, giá sẽ tự động bằng 0.</div>
                                </div>
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6 border-end">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-6 mb-3">
                                        ...
                                        <select name="type" id="edit_type" class="form-select" required>
                                            <option value="consumable">Hàng hóa (Bán đứt)</option>
                                            <option value="service">Dịch vụ (Thuê/Nhân sự)</option>
                                            <option value="amenities">Tiện ích (Wifi/WC)</option>
                                        </select>
                                    </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="d-flex align-items-center gap-3">
                                        <img id="edit_image_preview" src="" class="rounded border d-none"
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="alert alert-info small">
                                    <i class="fas fa-info-circle me-1"></i> Giá này là giá tham khảo chung. Để chỉnh giá
                                    chi tiết cho từng sân, vui lòng truy cập trang chi tiết Sân.
                                </div>

                                <div class="mb-3">
                                    <label class="form-label fw-bold text-success">Giá tham khảo (VNĐ)</label>
                                    {{-- step="1" để gợi ý nhập số nguyên --}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Xử lý khi đổi loại hình ở form thêm mới
            if (addTypeSelect) {
                addTypeSelect.addEventListener('change', function() {
                    togglePriceInput(this.value, addPriceInput);
                });
            }

            // Xử lý checkbox chọn tất cả trong form thêm mới
            var checkAllAdd = document.getElementById('checkAllAdd');
            if (checkAllAdd) {
                checkAllAdd.addEventListener('change', function() {
                    var checkboxes = document.querySelectorAll('.venue-checkbox-add');
                    checkboxes.forEach(chk => chk.checked = this.checked);
                });
            }


            // === LOGIC CHO MODAL EDIT ===
            ...
            if (modalEditService) {
                modalEditService.addEventListener('show.bs.modal', function(event) {
                    ...
                    // 1. CẬP NHẬT ACTION CỦA FORM
                    // Đường dẫn cập nhật: /owner/services/{id}
                    // Hãy chắc chắn route này khớp với định nghĩa trong web.php
                    var form = document.getElementById('formEditService');
                    form.action = '/owner/services/' + id;

                    // 2. ĐỔ DỮ LIỆU VÀO INPUT
                    ...
                    // 3. XỬ LÝ FORMAT GIÁ (Chuyển 12000.00 -> 12000)
                    var floatPrice = parseFloat(price);
                    var intPrice = Math.floor(floatPrice); // Làm tròn xuống để bỏ phần thập phân
                    var priceInput = document.getElementById('edit_price');
                    priceInput.value = intPrice;
                    ...
                    // 5. CHECK TYPE ĐỂ KHÓA/MỞ Ô GIÁ
                    togglePriceInput(type, priceInput);
                });

                // Lắng nghe sự kiện thay đổi Type trong Modal Edit
                document.getElementById('edit_type').addEventListener('change', function() {
                    var priceInput = document.getElementById('edit_price');
                    togglePriceInput(this.value, priceInput);
                });
            }

            // === HÀM DÙNG CHUNG: KHÓA/MỞ Ô GIÁ THEO TYPE ===

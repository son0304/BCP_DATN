...
<div class="container-fluid py-4">
    ...
    <div class="row">
            ...
            @if (isset($merchant_profile) && $merchant_profile)
                @php
                    ...
                    $statusConfig = match ($mStatus) {
                        ...
                        'pending'     => ['color' => 'warning text-dark', 'text' => 'Chờ xác thực', 'icon' => 'fa-clock'],
                        default       => ['color' => 'secondary', 'text' => 'Chờ xác thực', 'icon' => 'fa-question-circle'],
                    };
                @endphp
                ...
                <div class="card shadow-sm border-0 mb-4">
                    ...
                    <div class="card-body">
                        ...
                        @if($isProcessed)
                            <div class="text-center py-4">
                                <div class="mb-3">
                                    <i class="fas {{ $mStatus == 'approved' ? 'fa-check-circle text-success' : 'fa-times-circle text-danger' }}" style="font-size: 4rem;"></i>
                                </div>
                                ...
                                <a href="{{ route('admin.users.show', $venue->owner->id) }}" class="btn btn-primary px-4">
                        ...
                        @else
                            <dl class="row mb-0">
                                ...
                                <dd class="col-sm-8 font-monospace text-danger fw-bold">{{ $merchant_profile['bank_account_number'] ?? 'N/A' }}</dd>

                                <dt class="col-sm-4 text-muted">Chủ tài khoản:</dt>
                                <dd class="col-sm-8 text-uppercase">{{ $merchant_profile['bank_account_name'] ?? 'N/A' }}</dd>
                                ...
                                @if(!empty($merchant_profile['admin_note']) && $mStatus == 'resubmitted')
                                    <div class="col-12 mt-3">
                                        <div class="alert alert-info mb-0 border-0 small">
                                            ...
                                            <strong>Lý do từ chối lần trước:</strong> {{ $merchant_profile['admin_note'] }}
                                        </div>
                                    </div>
                                @endif
                            </dl>
                            ...
                            @if (isset($merchant_profile['images']) && count($merchant_profile['images']) > 0)
                                <div class="mt-4 pt-3 border-top">
                                    <h6 class="text-secondary small fw-bold mb-3"><i class="fas fa-file-contract me-1"></i> Giấy tờ đính kèm</h6>
                                    <div class="row g-2">
                                        @foreach ($merchant_profile['images'] as $image)
                                            <div class="col-4 col-md-3">
                                                <a href="{{ asset($image['url']) }}" target="_blank" class="d-block border rounded overflow-hidden shadow-sm position-relative group-hover">
                                                    <img src="{{ asset($image['url']) }}" class="img-fluid w-100" style="height: 100px; object-fit: cover;">
                                                    <div class="position-absolute bottom-0 w-100 bg-dark bg-opacity-75 text-white text-center small py-1">
                    ...
                    @if(!$isProcessed)
                        <div class="card-footer bg-light py-3 d-flex justify-content-end gap-2">
                            {{-- Nút Từ chối --}}
                            <button type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#rejectMerchantModal">
                            ...
                            <form action="{{ route('admin.venues.update-merchant', $venue->id) }}" method="POST">
                                ...
                                <button type="submit" class="btn btn-success" onclick="return confirm('Xác nhận thông tin doanh nghiệp hợp lệ?')">
            ...
            <div class="card shadow-sm border-0 mb-4">
                ...
                <div class="card-body">
                    <dl class="row mb-0">
                        ...
                        <dd class="col-sm-9">{{ $venue->address_detail }}, {{ $venue->district->name ?? '' }}, {{ $venue->province->name ?? '' }}</dd>
                        ...
                        <dd class="col-sm-9">
                             {{ \Carbon\Carbon::parse($venue->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($venue->end_time)->format('H:i') }}
                        </dd>
                    </dl>
                </div>
            </div>
                                ...
                                <tbody>
                                    @foreach ($venue->courts as $court)
                                        <tr>
                                            ...
                                            <td class="text-success fw-bold">{{ number_format($court->price_per_hour, 0, ',', '.') }}</td>
                                            <td>
                                                @if ($court->is_indoor)
                                                    <span class="badge bg-primary bg-opacity-10 text-primary">Trong nhà</span>
                                                @else
                                                    <span class="badge bg-success bg-opacity-10 text-success">Ngoài trời</span>
                                                @endif
                                            </td>
                                            <td class="text-end pe-3">
                                                <a href="{{ route('admin.venues.courts.show', ['venue' => $venue->id, 'court' => $court->id]) }}" class="btn btn-sm btn-light">
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body">
                    ...
                    <div class="d-grid gap-2">
                        @if(!$venue->is_active)
                            ...
                            <button type="button" class="btn btn-outline-danger w-100" data-bs-toggle="modal" data-bs-target="#rejectVenueModal">
                                <i class="fas fa-ban me-1"></i> Từ chối hoạt động
                            </button>
                        @else
                            {{-- Nếu đang active thì hiện nút Tạm dừng (Mở Modal) --}}
                            <button type="button" class="btn btn-warning w-100" data-bs-toggle="modal" data-bs-target="#rejectVenueModal">
                    ...
                    @if ($venue->images->count())
                        ...
                        <div class="row g-2">
                            @foreach ($others as $img)
                                <div class="col-4">
                                    <img src="{{ $img->url }}" class="w-100 rounded" style="height: 60px; object-fit: cover;">
                                </div>
                            @endforeach
                        </div>
                    @else
            ...
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white py-3 border-bottom">
                    <h6 class="mb-0 text-secondary">Dịch vụ</h6>
                </div>
                <div class="card-body p-0">
                    @if ($venue->services->count())
                        <ul class="list-group list-group-flush">
                            @foreach ($venue->services as $sv)
                                <li class="list-group-item d-flex justify-content-between">
                                    <span>{{ $sv->name }}</span>
                                    <span class="fw-bold">{{ number_format($sv->price) }}đ</span>
                                </li>
                            @endforeach
                        </ul>
                    @else
                        <div class="p-3 text-center text-muted small">Không có dịch vụ.</div>
                    @endif
                </div>
            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <textarea name="admin_note" class="form-control" rows="4" required placeholder="VD: Ảnh CCCD bị mờ, Tên chủ tài khoản không khớp với giấy phép..."></textarea>
                    </div>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="admin_note" class="form-control" rows="3" required placeholder="VD: Sân chưa hoàn thiện, Địa chỉ không có thực..."></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>

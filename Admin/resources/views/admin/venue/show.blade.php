...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            <form action="{{ route('admin.venue.updateStatus', $venue) }}" method="POST" class="d-inline">
                @csrf
                @method('PATCH')
            ...
            <a href="{{ route('venue.index') }}" class="btn btn-outline-secondary ms-2">
            ...
            <div class="card shadow-sm border-0 mb-4">
                ...
                <div class="card-body">
                    <dl class="row">
                        ...
                        <dd class="col-sm-9">{{ $venue->owner->name ?? 'N/A' }} ({{ $venue->owner->email ?? 'N/A' }})</dd>

                        <dt class="col-sm-3">Địa chỉ:</dt>
                        <dd class="col-sm-9">{{ $venue->address_detail }}, {{ $venue->district->name ?? '' }}, {{ $venue->province->name ?? '' }}</dd>

                        <dt class="col-sm-3">Giờ hoạt động:</dt>
                        <dd class="col-sm-9">{{ \Carbon\Carbon::parse($venue->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($venue->end_time)->format('H:i') }}</dd>
                    ...
                    @if ($venue->images->count())
                        ...
                        @if ($primaryImage)
                            <div class="mb-3 position-relative">
                                <img src="{{ $primaryImage->url }}" class="img-fluid rounded shadow-sm" alt="Ảnh chính">

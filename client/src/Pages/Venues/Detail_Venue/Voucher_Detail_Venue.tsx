...
interface VoucherProps {
  ...
  totalPrice: number;
  selectedVoucher: Voucher | null; // THÊM PROP NÀY: Để nhận dữ liệu từ cha
}
...
const Voucher_Detail_Venue: React.FC<VoucherProps> = ({
  ...
  onVoucherApply,
  totalPrice,
  selectedVoucher // Sử dụng prop này làm nguồn dữ liệu chính
}) => {
  // Chỉ giữ lại state UI
  const [voucherCode, setVoucherCode] = useState<string>("");
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [manualError, setManualError] = useState<string | null>(null);
  const [userInteracted, setUserInteracted] = useState(false); // Để chặn auto-apply nếu user đã tự thao tác
  ...
  const isValidVoucher = (voucher: Voucher, total: number) => {
    ...
    if (voucher.min_order_value > 0 && total < voucher.min_order_value) return false;
    if (voucher.usage_limit > 0 && voucher.used_count >= voucher.usage_limit) return false;

    return true;
  };

  // --- TÍNH TOÁN GIẢM GIÁ (Để hiển thị UI) ---
  ...
  // Chỉ chạy 1 lần khi load hoặc khi giá thay đổi mà chưa có voucher
  useEffect(() => {
    if (!userInteracted && !selectedVoucher && totalPrice > 0 && availableVouchers.length > 0) {
      ...
      if (validVouchers.length > 0) {
        ...
        const bestVoucher = validVouchers.reduce((prev, current) => {
          const prevDiscount = calculateDiscount(prev, totalPrice);
          const currDiscount = calculateDiscount(current, totalPrice);
          return (currDiscount > prevDiscount) ? current : prev;
        });

        if (calculateDiscount(bestVoucher, totalPrice) > 0) {
          onVoucherApply(bestVoucher);
          setVoucherCode(bestVoucher.code);
        }
      }
    }
  }, [availableVouchers, totalPrice]); // Bỏ selectedVoucher khỏi dependency để tránh loop nếu cha update lại
  ...
  const handleSelectFromDropdown = (voucher: Voucher) => {
    ...
    if (isValidVoucher(voucher, totalPrice)) {
      ...
      onVoucherApply(voucher); // Gọi cha update
      setIsDropdownOpen(false);
    } else {
      setVoucherCode(voucher.code);
  ...
  // Sync input code khi selectedVoucher thay đổi từ bên ngoài (hoặc auto apply)
  useEffect(() => {
    if (selectedVoucher) {
    ...
    <div className="flex flex-col gap-4 p-4 bg-white rounded-xl border border-gray-100 shadow-sm" ref={dropdownRef}>
          ...
          {selectedVoucher ? (
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-gray-800 truncate">{selectedVoucher.code}</p>
                <p className="text-xs text-gray-500 truncate">
                  {selectedVoucher.type === 'percentage'
                    ? `Giảm ${parseFloat(selectedVoucher.value)}%`
                    : `Giảm ${formatPrice(parseFloat(selectedVoucher.value))}`}
                </p>
              </div>
            </div>
          ) : (
          ...
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-xl shadow-xl z-50 max-h-[300px] overflow-y-auto custom-scrollbar animate-fadeIn">
            <div className="p-2 space-y-2">
              {availableVouchers.length > 0 ? availableVouchers.map((v) => {
                ...
                const discountText = v.type === 'percentage' ? `${valNum}%` : formatPrice(valNum);
      ...
      <div className="flex gap-2 pt-2 border-t border-gray-100">
        <input
          ...
          className="flex-1 w-full pl-3 pr-8 py-2.5 text-sm border border-gray-200 rounded-lg focus:border-[#10B981] outline-none uppercase bg-gray-50 focus:bg-white transition-all"
        />
        <button
          onClick={handleApplyManual}
          type="button"
          disabled={!voucherCode}
          className="px-5 py-2.5 bg-gray-900 hover:bg-[#10B981] text-white text-sm font-bold rounded-lg transition-all disabled:bg-gray-300 disabled:cursor-not-allowed"
        >
          Áp dụng
        </button>
      </div>
      ...
      {manualError && (
        <div className="p-2 bg-red-50 text-red-600 text-xs rounded-lg flex items-center gap-2 animate-pulse">
          <i className="fa-solid fa-circle-exclamation"></i> {manualError}
        </div>
      )}

      {selectedVoucher && !manualError && isValidVoucher(selectedVoucher, totalPrice) && (
        <div className="p-3 bg-[#10B981]/10 border border-[#10B981]/20 rounded-lg flex flex-col gap-1 animate-fadeIn">
          <div className="flex justify-between items-center">
            <span className="text-xs font-bold text-gray-600">Mã: {selectedVoucher.code}</span>
            <span className="text-xs font-bold text-[#10B981]">
              -{formatPrice(calculateDiscount(selectedVoucher, totalPrice))}
            </span>
          </div>
          {selectedVoucher.max_discount_amount > 0 && selectedVoucher.type === 'percentage' && (
            <p className="text-[10px] text-orange-600">
              *Tối đa: {formatPrice(selectedVoucher.max_discount_amount)}
            </p>
          )}
        </div>
      )}

...
<div class="main-content bg-light pb-5" style="min-height: 100vh;">
    ...
    <div class="container-fluid px-4 py-4">
        <form action="{{ route('owner.flash_sale_campaigns.store') }}" method="POST" id="flashSaleForm" onsubmit="return false;">
            @csrf
                            ...
                            <div class="card-body p-4">
                                <div class="mb-3">
                                    ...
                                    <div class="fw-bold small">
                                        ...
                                        {{ \Carbon\Carbon::parse($campaign->end_datetime)->format('H:i d/m') }}
                                    </div>
                                </div>
                                ...
                                <button type="button" class="btn btn-primary w-100 py-2 fw-bold shadow"
                                    id="btnSubmit"
                                    onclick="window.checkAndSubmit()"
                                    disabled>
                                    Xác nhận & Lưu
                                </button>
                            </div>
...
<style>
    ...
    .slot-item.price-invalid .slot-box {
        border-color: #dc3545 !important;
        background-color: #fff8f8;
    }

    .slot-item.price-invalid .old {
        color: #dc3545 !important;
        font-weight: bold;
    }

    .slot-item.price-invalid .new {
        font-size: 0.75rem;
    }
</style>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const mainForm = document.getElementById('flashSaleForm');

        // Hàm kiểm tra và gửi form (Dùng cho cả 2 nút)
        window.checkAndSubmit = function() {
            const price = parseFloat(priceInput.value) || 0;
            const checkedBoxes = document.querySelectorAll('.slot-checkbox:checked');
            const checkedCount = checkedBoxes.length;

            // 1. Kiểm tra số lượng sân đã chọn
            if (checkedCount === 0) {
                alert('Vui lòng chọn ít nhất một sân trước khi lưu!');
                return false;
            }

            // 2. Kiểm tra giá
            if (price <= 0) {
                alert('Vui lòng nhập giá giảm hợp lệ (lớn hơn 0)!');
                priceInput.focus();
                return false;
            }

            // 3. Kiểm tra xem có sân nào bị lỗi giá (đỏ) không
            let hasError = false;
            checkedBoxes.forEach(cb => {
                const slotItem = cb.closest('.slot-item');
                if (slotItem.classList.contains('price-invalid')) {
                    hasError = true;
                }
            });

            if (hasError) {
                alert('Có sân đang chọn có giá giảm cao hơn giá gốc. Vui lòng kiểm tra lại các ô màu đỏ!');
                return false;
            }

            // Nếu mọi thứ OK thì mới submit
            mainForm.submit();
        };
        ...
        const updateUI = () => {
            ...
            checkboxes.forEach(cb => {
                const slotItem = cb.closest('.slot-item');
                const preview = slotItem.querySelector('.sale-preview');
                const oldPriceText = slotItem.querySelector('.old').textContent.replace(/k|,|đ/g, '');
                const originalPrice = parseFloat(oldPriceText) * 1000;

                const isInvalid = price > originalPrice;

                if (isInvalid) {
                    preview.textContent = 'Lỗi giá';
                    preview.classList.add('text-danger');
                    slotItem.classList.add('price-invalid');
                    if (cb.checked) hasError = true;
                } else {
                    preview.textContent = price > 0 ? (price / 1000) + 'k' : '--';
                    preview.classList.remove('text-danger');
                    slotItem.classList.remove('price-invalid');
                }
            ...
            // Chốt chặn logic
            if (hasError) {
                pricePreview.innerHTML = '<span class="text-danger small">Giá sale > Giá gốc!</span>';
                btnSubmit.disabled = true;
            } else {
                pricePreview.classList.remove('text-danger');
        ...
        document.querySelectorAll('.select-all-court').forEach(sw => {
            sw.addEventListener('change', function() {
                this.closest('.tab-pane').querySelectorAll('.slot-checkbox').forEach(cb => {
